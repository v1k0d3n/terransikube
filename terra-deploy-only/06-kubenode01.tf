...
# Brandon B. Jozsa
#
/* Kubenode01 Deployment */
resource "openstack_compute_instance_v2" "kubenode01" {
  user_data = <<EOF
#cloud-config
hostname: kubenode01.kubelab.com
...
EOF
  ...
  floating_ip = "${openstack_compute_floatingip_v2.kubenode01_dev_float.address}"
  network {
    uuid = "${openstack_networking_network_v2.ext_net.id}"
    fixed_ip_v4 = "${var.kubenode01_fixed_ipv4}"
  }
  connection {
  ...
  provisioner "local-exec" {
    command = "echo ${openstack_compute_floatingip_v2.kubenode01_dev_float.address} kubenode01.kubelab.com kubenode01 >> hosts-out"
  }
  provisioner "file" {
    source = "kubenode"
