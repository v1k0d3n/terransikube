...
# Brandon B. Jozsa
#
/* Kubenode05 Deployment */
resource "openstack_compute_instance_v2" "kubenode05" {
  user_data = <<EOF
#cloud-config
hostname: kubenode05.kubelab.com
...
EOF
  ...
  floating_ip = "${openstack_compute_floatingip_v2.kubenode05_dev_float.address}"
  network {
    uuid = "${openstack_networking_network_v2.ext_net.id}"
    fixed_ip_v4 = "${var.kubenode05_fixed_ipv4}"
  }
  connection {
  ...
  provisioner "local-exec" {
    command = "echo ${openstack_compute_floatingip_v2.kubenode05_dev_float.address} kubenode05.kubelab.com kubenode05 >> hosts-out"
  }
  provisioner "file" {
    source = "kubenode"
