...
# Brandon B. Jozsa
#
/* Kubenode02 Deployment */
resource "openstack_compute_instance_v2" "kubenode02" {
  user_data = <<EOF
#cloud-config
hostname: kubenode02.kubelab.com
...
EOF
  ...
  floating_ip = "${openstack_compute_floatingip_v2.kubenode02_dev_float.address}"
  network {
    uuid = "${openstack_networking_network_v2.ext_net.id}"
    fixed_ip_v4 = "${var.kubenode02_fixed_ipv4}"
  }
  connection {
  ...
  provisioner "local-exec" {
    command = "echo ${openstack_compute_floatingip_v2.kubenode02_dev_float.address} kubenode02.kubelab.com kubenode02 >> hosts-out"
  }
  provisioner "file" {
    source = "kubenode"
